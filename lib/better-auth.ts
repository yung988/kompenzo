...
export function BetterAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Nepodařilo se načíst uživatelská data');
        setUser(null);
      } finally {
        setIsLoading(false);
      }
  ...
  const login = async (email: string, password: string): Promise<User | null> => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Došlo k chybě při přihlašování');
      return null;
    } finally {
      setIsLoading(false);
    }
  ...
  const register = async (
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Došlo k chybě při registraci');
      return null;
    } finally {
      setIsLoading(false);
    }
  ...
  const logout = async (): Promise<boolean> => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Došlo k chybě při odhlašování');
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const updateProfile = async (
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Došlo k chybě při aktualizaci profilu');
      return null;
    } finally {
      setIsLoading(false);
    }
